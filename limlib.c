...
A lua library to work with images, documentation in file: DOCUMENTATION.mkd

Updated: October 27th, 2016

Compile this code in a terminal (Ex. I use 'gcc -o imlib imlib.c `imlib2-config --cflags` `imlib2-config --libs` -lm -L/usr/local/lib -llua')

Run this code using: './imlib *name of lua file*' (Ex. I use './imlib first')

Requirements:
-imlib C library
-Lua installation (5.3.3 used here)
...
static Imlib_Color *push_Color(lua_State *L) {
  ...
  luaL_getmetatable(L, "imlib.color");
  lua_setmetatable(L, -2);
  return c;
}

#define check_color(L,n)\
  (Imlib_Color*)luaL_checkudata(L, n, "imlib.color")
...
static int colorm_tostring(lua_State *L) {
  ...
  lua_pushfstring(L, "<imlib.color r=%d g=%d b=%d a=%d> (%p)",
...
static Imlib_Border *push_Border(lua_State *L) {
  ...
  luaL_getmetatable(L, "imlib.border");
  lua_setmetatable(L, -2);
  return b;
}
...
#define check_border(L, n)\
  (Imlib_Border*)luaL_checkudata(L, n, "imlib.border")

/* bd:__tostring() */
static int borderm_tostring(lua_State *L) {
  Imlib_Border *b = check_border(L, 1);
  lua_pushfstring(L, "<imlib.border left=%d top=%d right=%d bottom=%d> (%p)",
...
static Gradient push_Gradient(lua_State *L, Gradient gr) {
  ...
  *grp = gr;
  luaL_getmetatable(L, "imlib.gradient");
  lua_setmetatable(L, -2);
  return gr;
}
...
static Gradient check_Gradient(lua_State *L, int n) {
  Gradient *grp, gr;
  grp = (Gradient*)luaL_checkudata(L, n, "imlib.gradient");
  gr = *grp;
  if (gr == NULL)
...
static int gradientm_gc(lua_State *L) {
  Gradient *grp = (Gradient*)luaL_checkudata(L, 1, "imlib.gradient");
  Gradient gr = *grp;
  if (gr) {
...
static int gradientm_tostring(lua_State *L) {
  ...
  Gradient gr = *grp;
  if (gr)
    lua_pushfstring(L, "<imlib.gradient> (%p)", gr);
  else
    lua_pushfstring(L, "<imlib.gradient> (freed)");
  return 1;
}
...
static Polygon push_Polygon(lua_State *L, Polygon po) {
  ...
  *pop = po;
  luaL_getmetatable(L, "imlib.polygon");
  lua_setmetatable(L, -2);
  return po;
}
...
static Polygon check_Polygon(lua_State *L, int n) {
  Polygon *pop, po;
  pop = (Polygon*)luaL_checkudata(L, n, "imlib.polygon");
  po = *pop;
  if (po == NULL)
...
static int polygonm_gc(lua_State *L) {
  Polygon *pop = (Polygon*)luaL_checkudata(L, 1, "imlib.polygon");
  Polygon po = *pop;
  if (po) {
...
static int polygonm_tostring(lua_State *L) {
  ...
  if (po)
    lua_pushfstring(L, "<imlib.polygon> (%p)", po);
  else
    lua_pushfstring(L, "<imlib.polygon> (freed)");
  return 1;
}
...
static Font push_Font(lua_State *L, Font fo) {
  ...
  *fop = fo;
  luaL_getmetatable(L, "imlib.font");
  lua_setmetatable(L, -2);
  return fo;
}

static Font check_Font(lua_State *L, int n) {
  Font *fop = luaL_checkudata(L, n, "imlib.font");
  Font fo = *fop;
  if (fo == NULL)
...
static int font_add_path(lua_State *L) {
  const char *path = luaL_checkstring(L, 1);
  /* This is safe. imlib does a strdup */
...
static int fontm_gc(lua_State *L) {
  Font *fop = (Font*)luaL_checkudata(L, 1, "imlib.font");
  Font fo = *fop;
  if (fo) {
...
static int fontm_tostring(lua_State *L) {
  ...
  Font fo = *fop;
  if (fo)
    lua_pushfstring(L, "<imlib.font> (%p)", fo);
  else
    lua_pushfstring(L, "<imlib.font> (freed)");
  return 1;
}
...
static Image push_Image(lua_State *L, Image im) {
  ...
  *imp = im;
  luaL_getmetatable(L, "imlib.image");
  lua_setmetatable(L, -2);
  return im;
}
...
static Image check_Image(lua_State *L, int n) {
  Image *imp, im;
  imp = (Image*)luaL_checkudata(L, n, "imlib.image");
  im = *imp;
  if (im == NULL)
...
static int imagem_tostring(lua_State *L) {
  Image *imp = (Image*)luaL_checkudata(L, 1, "imlib.image");
  Image im = *imp;
  if (im == NULL) {
    lua_pushfstring(L, "<imlib.image> (freed)");
  } else {
    imlib_context_set_image(im);
    lua_pushfstring(L, "<imlib.image width=%d height=%d> (%p)",
...
static int imagem_set_border(lua_State *L) {
  Image im = check_Image(L, 1);
  Imlib_Border *b = (Imlib_Border*)luaL_checkudata(L, 2, "imlib.border");
  imlib_context_set_image(im);
...
/*** imlib.image metamethods which wrap functions that return copies (but are  
...
static int imagem_draw_pixel(lua_State *L) {
  ...
  if (lua_gettop(L)>=4) { /* given a colour */
    Imlib_Color *c = (Imlib_Color*)luaL_checkudata(L, 4, "imlib.color");
    set_color(c);
  }
  imlib_image_draw_pixel(x, y, 0); /* Buggy for imlib <= 1.0.5 apparently */
  return 0;
}
...
static int imagem_draw_line(lua_State *L) {
  ...
  if (lua_gettop(L)>=6) /* Given a color */ {
    Imlib_Color *c = (Imlib_Color*)luaL_checkudata(L, 6, "imlib.color");
    set_color(c);
  }
...
static struct luaL_Reg border_f [] = {
  {"new", border_new},
  {NULL, NULL}
};

static struct luaL_Reg border_m [] = {
...
static struct luaL_Reg color_f [] = {
  {"new", color_new},
  {NULL, NULL}
};

static struct luaL_Reg color_m [] = {
...
static struct luaL_Reg gradient_f [] = {
  {"new", gradient_new},
  {NULL, NULL}
};

static struct luaL_Reg gradient_m [] = {
  {"__gc", gradientm_gc},
...
static struct luaL_Reg polygon_f [] = {
  {"new", polygon_new},
  {NULL, NULL}
};

static struct luaL_Reg polygon_m [] = {
  {"__gc", polygonm_gc},
...
int luaopen_border (lua_State *L) {
...
int luaopen_limlib(lua_State *L) {
  ...
  luaL_requiref(L,"border",luaopen_border,1);
  luaL_requiref(L,"color",luaopen_color,1);
  luaL_requiref(L,"gradient",luaopen_gradient,1);
  luaL_requiref(L,"polygon",luaopen_polygon,1);
